...
parser.add_argument("--img_path", type=str, default= './dataset/moving/test_distorted/22.png')
parser.add_argument("--flow_path", type=str, default= './my_array.npy')
...
file_loc = "E:\迅雷下载\elastix-5.1.0-win64\moving"
save_loc = r"E:\迅雷下载\elastix-5.1.0-win64\result\GeoNetResult"
for i in range(9,10):
    filename = os.path.join(file_loc,str(i)+".png")
    print(filename)
    img = Image.open(filename)
    img = img.resize((256,256))
    distortedImg = io.imread(filename)
    print(type(distortedImg))
    print(type(img))
    flow = np.load(args.flow_path,allow_pickle=True)
    resImg, resMsk = rectification(np.asarray(img), flow)
    savename = os.path.join(save_loc,str(i)+".png")
    print(savename)
    io.imsave(savename,resImg)
